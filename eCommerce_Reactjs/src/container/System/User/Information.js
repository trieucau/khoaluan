...
const Information = () => {
  ...
  const [values, setValues] = useState({ firstName:'', lastName:'', address:'', phonenumber:'', genderId:'', dob:'', roleId:'', email:'', image: DEFAULT_AVATAR, isActiveEmail:'', imageReview:'' });

  useEffect(() => {
    getDetailUserById(id).then(res => {
      if (res?.errCode === 0) {
        const d = res.data;
        setValues({ firstName: d.firstName, lastName: d.lastName, address: d.address, phonenumber: d.phonenumber, genderId: d.genderId, roleId: d.roleId, email: d.email, id: d.id, dob: d.dob, image: d.image || DEFAULT_AVATAR, isActiveEmail: d.isActiveEmail, imageReview: '' });
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
      const res = await UpdateUserService({ id, firstName: values.firstName, lastName: values.lastName, address: values.address, roleId: values.roleId, genderId: values.genderId, phonenumber: values.phonenumber, dob: isChangeDate ? new Date(birthday).getTime() : values.dob, image: values.image });
          ...
          <div className="ap-card-body" style={{ textAlign: 'center', padding: 28 }}>
            <div style={{ position: 'relative', display: 'inline-block', marginBottom: 16 }}>
              {displayImg && displayImg !== DEFAULT_AVATAR ? (
                <img src={displayImg} alt="avatar" onClick={() => setLightboxOpen(true)}
                  style={{ width: 120, height: 120, borderRadius: '50%', objectFit: 'cover', border: '3px solid var(--ap-primary)', cursor: 'zoom-in' }} />
              ) : (
                <div style={{ width: 120, height: 120, borderRadius: '50%', background: 'linear-gradient(135deg, var(--ap-primary), #818cf8)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 40, fontWeight: 800, color: '#fff', border: '3px solid var(--ap-primary)', margin: '0 auto' }}>
                  {initials || '?'}
                </div>
              )}
              <input type="file" id="avatarImg" accept=".jpg,.png,.webp" hidden onChange={handleImageChange} />
              <label htmlFor="avatarImg" style={{ position: 'absolute', bottom: 0, right: 0, background: 'var(--ap-primary)', borderRadius: '50%', width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 14, border: '2px solid var(--ap-surface)' }}>📷</label>
            </div>
            <div style={{ fontWeight: 700, fontSize: 16 }}>{values.firstName} {values.lastName}</div>
            ...
            {values.isActiveEmail === 0 && (
              <button className="ap-btn ap-btn-ghost" style={{ marginTop: 10, fontSize: 12 }} onClick={handleSendEmail}>
                📧 Gửi email xác thực
              </button>
            )}
        ...
        <div className="ap-card">
          ...
          <div className="ap-card-body">
            ...
            <div className="ap-form-row">
              ...
              <div className="ap-form-group">
                ...
                <div className="ap-datepicker-wrap" style={{ display: 'block' }}>
                  <DatePicker className="ap-input" selected={birthday} dateFormat="dd/MM/yyyy"
            ...
            <div className="ap-form-group">
              ...
              <input className="ap-input" name="address" value={values.address} onChange={handleChange} placeholder="123 Đường ABC, Q.1..." />
            </div>
            <div className="ap-form-group">
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
              ...
              <Link to={`/admin/change-password/${id}`} className="ap-btn ap-btn-ghost">🔐 Đổi mật khẩu</Link>
            </div>
          </div>
        </div>
      </div>
      {lightboxOpen && <Lightbox slides={[{ src: displayImg }]} open={lightboxOpen} close={() => setLightboxOpen(false)} />}
    </div>
  );
};
