...
const ManageVoucher = () => {
  ...
  const handleDelete = async (id) => {
    ...
    if (res?.errCode === 0) {
      ...
      fetchData(page * PAGINATION.pagerow);
  ...
  return (
    <div className="ap-page">
      <PageHeader
        title={
          <>
            <i className="fa-solid fa-tags" style={{ marginRight: 8 }}></i>Quản lý mã Voucher
          </>
        }
        subtitle="Danh sách mã giảm giá và khuyến mãi"
        actions={
          <>
            ...
            <Link to="/admin/add-voucher" className="ap-btn ap-btn-primary">
              + Thêm voucher
            </Link>
          </>
        }
      ...
      <div className="ap-card">
            ...
            <tbody>
              {loading ? (
                ...
                <EmptyState
                  icon={
                    <>
                      <i className="fa-solid fa-tags" style={{ marginRight: 8 }}></i>
                    </>
                  }
                ...
                data.map((item, idx) => {
                  ...
                  const status = expired
                    ? { label: 'Hết hạn', cls: 'ap-badge-red' }
                    : used
                      ? { label: 'Hết lượt', cls: 'ap-badge-amber' }
                      : { label: 'Đang dùng', cls: 'ap-badge-green' };
                  return (
                    <tr
                      key={item.id}
                    ...
                    >
                      ...
                      <td>
                        <span
                          style={{
                            color: item.usedAmount >= item.amount ? '#fca5a5' : '#6ee7b7',
                            fontWeight: 600,
                          }}
                        >
                          {item.usedAmount}
                      ...
                      <td style={{ fontSize: 12, color: 'var(--ap-text-muted)' }}>
                        {moment.unix(item.fromDate / 1000).format('DD/MM/YYYY')}
                      </td>
                      <td
                        style={{
                          fontSize: 12,
                          color: expired ? '#fca5a5' : 'var(--ap-text-muted)',
                        }}
                      >
                        {moment.unix(item.toDate / 1000).format('DD/MM/YYYY')}
                      </td>
                      <td>
        ...
        <AdminPagination
          ...
          onPageChange={({ selected }) => {
            setPage(selected);
            fetchData(selected * PAGINATION.pagerow);
          }}
        />
      </div>
    </div>
