  ...
  return (
    <div className="ap-page">
      <PageHeader
        title={
          <>
            <i className="fa-solid fa-ticket" style={{ marginRight: 8 }}></i>Quản lý loại khuyến mãi
          </>
        }
        subtitle="Cấu hình các loại voucher giảm giá"
        actions={
          <>
            ...
            <Link to="/admin/add-typevoucher" className="ap-btn ap-btn-primary">
              + Thêm loại KM
            </Link>
          </>
        }
            ...
            <tbody>
              {loading ? (
                ...
                <EmptyState
                  ...
                  title="Không có loại khuyến mãi"
                />
              ) : (
                data.map((item, idx) => (
                  <tr
                  ...
                  >
                    ...
                    <td>
                      <span
                        className={`ap-badge ${item.typeVoucher === 'percent' ? 'ap-badge-indigo' : 'ap-badge-amber'}`}
                      >
                    ...
                    <td style={{ fontSize: 13, color: 'var(--ap-text-muted)' }}>
                      {CommonUtils.formatter.format(item.minValue)}
                    </td>
                    <td style={{ fontSize: 13, color: 'var(--ap-text-muted)' }}>
                      {CommonUtils.formatter.format(item.maxValue)}
                    </td>
                    <td>
                      <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
                        <Link
                          to={`/admin/edit-typevoucher/${item.id}`}
