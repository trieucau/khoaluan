...
const PAYMENT_BADGE = { 0: { label: 'Tiền mặt', cls: 'ap-badge-gray' }, 1: { label: 'Online', cls: 'ap-badge-green' } };
      ...
      <tr key={i}>
        {Array(cols).fill(0).map((_, j) => (
          <td key={j} style={{ padding: '13px 14px' }}>
            <div className="ap-skeleton ap-skeleton-text" style={{ width: j === 0 ? '50%' : '70%' }} />
          </td>
        ))}
      </tr>
    ))}
...
const ManageOrder = () => {
  ...
  const loadOrderData = useCallback(async (sid = statusId, offset = 0) => {
    ...
    try {
      ...
      if (res?.errCode === 0) { setDataOrder(res.data); setCount(Math.ceil(res.count / PAGINATION.pagerow)); }
    } catch {}
  ...
  const filtered = search
    ? dataOrder.filter(o => String(o.id).includes(search) || o.userData?.email?.toLowerCase().includes(search.toLowerCase()) || o.userData?.phonenumber?.includes(search))
    : dataOrder;
      ...
      <div className="ap-card">
        ...
        <div className="ap-table-wrap">
          <table className="ap-table">
            ...
            <tbody>
              {loading ? <SkeletonRows cols={10} /> : filtered.length === 0 ? (
                <tr><td colSpan={10} style={{ padding: 0, border: 'none' }}>
                  <div className="ap-empty"><div className="ap-empty-icon">📭</div>
                ...
                const pay = PAYMENT_BADGE[item.isPaymentOnlien] || { label: '—', cls: 'ap-badge-gray' };
                return (
                  <tr key={item.id} className="ap-row-enter" style={{ animationDelay: `${idx * 30}ms` }}>
                    ...
                    <td style={{ color: 'var(--ap-text-muted)', fontSize: 13 }}>{item.userData?.phonenumber}</td>
                    ...
                    <td style={{ fontSize: 12, color: 'var(--ap-text-muted)' }}>{item.voucherData?.codeVoucher || '—'}</td>
                    <td><span className={`ap-badge ${pay.cls}`}>{pay.label}</span></td>
                    <td><span className={`ap-badge ${STATUS_BADGE[item.statusId] || 'ap-badge-gray'}`}>{item.statusOrderData?.value}</span></td>
