...
const InfoRow = ({ label, value, highlight }) => (
  ...
  >
    ...
    <span
      style={{ fontWeight: highlight ? 700 : 500, color: highlight ? '#fbbf24' : 'var(--ap-text)' }}
    >
      {value}
    </span>
  </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {[1, 2, 3].map((i) => (
            <div
              ...
              style={{ height: 120, borderRadius: 'var(--ap-radius)' }}
            />
          ))}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20 }}>
          ...
          <div className="ap-card">
            <div className="ap-card-header">
              <span className="ap-card-title">
                <i className="fa-solid fa-cart-shopping" style={{ marginRight: 8 }}></i>Sản phẩm
                trong đơn
              </span>
            </div>
                  ...
                  {order.orderDetail?.map((item, idx) => {
                    ...
                    return (
                      <tr key={idx}>
                        ...
                        <td style={{ fontSize: 13, color: 'var(--ap-text-dim)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div className="ap-card">
            <div className="ap-card-header">
              <span className="ap-card-title">
                <i className="fa-solid fa-list-check" style={{ marginRight: 8 }}></i>Trạng thái đơn
                hàng
              </span>
            </div>
            <div className="ap-card-body">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                <span
                  className={`ap-badge ${STATUS_BADGE[order.statusId] || 'ap-badge-gray'}`}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                {order.statusId === 'S4' && (
                  <button
                    className="ap-btn ap-btn-primary"
                    style={{ width: '100%', justifyContent: 'center' }}
                    onClick={() => updateStatus('S5')}
                  >
                    <i className="fa-solid fa-box" style={{ marginRight: 8 }}></i>Xác nhận gửi hàng
                  </button>
                )}
                {order.statusId === 'S5' && (
                  <button
          ...
          <div className="ap-card">
            <div className="ap-card-header">
              <span className="ap-card-title">
                <i className="fa-solid fa-user" style={{ marginRight: 8 }}></i>Thông tin nhận hàng
              </span>
            </div>
            <div className="ap-card-body">
          ...
          <div className="ap-card">
            ...
            <div className="ap-card-body">
              ...
              <InfoRow
                label="Loại ship"
                value={`${order.typeShipData?.type} — ${CommonUtils.formatter.format(shipPrice)}`}
              />
              {order.voucherId && (
                <InfoRow label="Voucher" value={order.voucherData?.codeVoucher} />
              )}
              <InfoRow label="Tổng hàng" value={CommonUtils.formatter.format(subtotal)} />
              {discount > 0 && (
                <InfoRow label="Giảm giá" value={`-${CommonUtils.formatter.format(discount)}`} />
              )}
              <InfoRow
                label="Tổng thanh toán"
                value={CommonUtils.formatter.format(subtotal - discount + shipPrice)}
                highlight
              />
            </div>
          </div>
          ...
          {order.shipperData && (
            <div className="ap-card">
              <div className="ap-card-header">
                <span className="ap-card-title">
                  <i className="fa-solid fa-truck-fast" style={{ marginRight: 8 }}></i>Shipper
                </span>
              </div>
              <div className="ap-card-body">
                <InfoRow
                  label="Tên"
                  value={`${order.shipperData.firstName} ${order.shipperData.lastName}`}
                />
