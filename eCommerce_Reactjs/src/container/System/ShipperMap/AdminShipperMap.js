...
const AdminShipperMap = ({ isMini = false }) => {
  ...
  useEffect(() => {
    waitForLeaflet(() => {
      ...
      const map = window.L.map(mapDivRef.current, { zoomControl: true }).setView([10.8, 106.6], 11); // default: Hồ Chí Minh
  ...
  useEffect(() => {
    ...
    getAdminShippersOnMap()
      .then((res) => {
        if (res?.errCode === 0 && res.data) setList(res.data);
      })
      .catch(() => {})
    ...
    displayList.forEach((s, idx) => {
      ...
      if (routeCoords.length > 1) {
        ...
        if (osrmCache[cacheKey]) {
          const polyline = window.L.polyline(osrmCache[cacheKey], {
            color: color,
            weight: 5,
            opacity: 0.85,
          }).addTo(map);
          ...
          const fallbackLine = window.L.polyline(routeCoords, {
            color: color,
            weight: 4,
            opacity: 0.4,
            dashArray: '5, 10',
          ...
          osrmQueue.push({
            ...
            callback: (pathCoords) => {
              ...
              const finalLine = window.L.polyline(finalCoords, {
                color: color,
                weight: 5,
                opacity: 0.85,
  ...
  const mapContent = (
    <div
      style={{
        display: 'flex',
        height: isMini ? 320 : '75vh',
        minHeight: isMini ? 320 : 600,
        width: '100%',
        overflow: 'hidden',
        position: 'relative',
        borderRadius: isMini ? 0 : 'var(--ap-radius)',
      }}
    >
      {/* Map Container */}
      <div
        ref={mapDivRef}
        style={{ height: '100%', width: '100%', zIndex: 0, isolation: 'isolate' }}
      />
        ...
        {!isMini && (
          <button
            ...
            style={{
              ...
              color: '#fff',
              border: 'none',
              borderRadius: isPanelOpen && isMobile ? '8px 0 0 8px' : '0 8px 8px 0',
              cursor: 'pointer',
              fontSize: 18,
              fontWeight: 700,
              boxShadow: '2px 0 12px rgba(59,130,246,0.4)',
              transition: 'left 0.35s cubic-bezier(.4,0,.2,1), border-radius 0.3s',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              pointerEvents: 'auto',
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            cursor: 'pointer',
                            marginBottom: 10,
                            background: idx % 2 === 0 ? '#111827' : '#1e293b',
                            ...
                            <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                              <span
                                style={{
                                  ...
                                  color: hasGps ? '#34d399' : '#94a3b8',
                                }}
                              >
                                {hasGps ? 'GPS On' : 'Offline'}
                              </span>
                              <span
                                style={{
                                  fontSize: 10,
                          ...
                          return (
                            ...
                            >
                              ...
                              <td style={{ padding: '9px 10px', whiteSpace: 'nowrap' }}>
                                <button
                                  onClick={() =>
                                    setSelectedOrderId(selectedOrderId === oid ? null : oid)
                                  }
                                  style={{
                                    background: selectedOrderId === oid ? '#3b82f6' : 'transparent',
                                    border: '1px solid #3b82f6',
                                    color: selectedOrderId === oid ? '#fff' : '#38bdf8',
                                    borderRadius: 4,
                                    padding: '3px 8px',
                                    cursor: 'pointer',
                                    fontSize: 11,
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button
              ...
              onClick={() => {
                ...
                getAdminShippersOnMap()
                  .then((res) => {
                    if (res?.errCode === 0) setList(res.data);
                  })
                  .finally(() => setLoading(false));
              }}
            >
