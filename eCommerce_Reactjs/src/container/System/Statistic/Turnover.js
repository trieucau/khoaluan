...
const STATUS_BADGE = {
  'Chờ xác nhận': 'ap-badge-amber',
  'Đang xử lý': 'ap-badge-cyan',
  'Đang giao': 'ap-badge-indigo',
  'Đã giao': 'ap-badge-green',
  'Đã hủy': 'ap-badge-red',
};
  ...
  return (
    <div className="ap-page">
      <PageHeader
        title={
          <>
            <i className="fa-solid fa-chart-line" style={{ marginRight: 8 }}></i>Thống kê doanh thu
          </>
        }
        subtitle="Lọc và phân tích doanh thu theo ngày / tháng / năm"
        actions={
          data.length > 0 && (
            <button
              className="ap-btn ap-btn-success"
              onClick={() => CommonUtils.exportExcel(dataExport, 'Doanh thu', 'TurnOver')}
            >
          ...
          {[
            {
              ...
              desc: `${stats.deliveredOrders} đơn đã giao`,
              value: CommonUtils.formatter.format(stats.actual),
            ...
            },
            {
              ...
              desc: `${stats.totalOrders - stats.deliveredOrders - stats.cancelledOrders} đơn đang xử lý`,
              value: CommonUtils.formatter.format(stats.pending),
              icon: <i className="fa-solid fa-hourglass-half"></i>,
              color: '#f59e0b',
              bg: 'rgba(245, 158, 11, 0.15)',
            },
            {
              label: 'Khách Hủy (Thất Thoát)',
              desc: `${stats.cancelledOrders} đơn bị hủy`,
              value: CommonUtils.formatter.format(stats.cancelled),
            ...
            },
            {
              ...
              desc: 'Chỉ tính đơn thành công',
              value: CommonUtils.formatter.format(
                stats.deliveredOrders > 0 ? Math.round(stats.actual / stats.deliveredOrders) : 0
              ),
            ...
            <tbody>
              {loading ? (
                ...
                <EmptyState
                  ...
                  desc="Chưa có dữ liệu thống kê trong khoảng thời gian này"
                />
              ) : (
                data.map((item, idx) => {
                  const status = item.statusOrderData?.value;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td>
                        <span
                          className={`ap-badge ${item.isPaymentOnlien === 0 ? 'ap-badge-gray' : 'ap-badge-cyan'}`}
                        >
                          {item.isPaymentOnlien === 0 ? (
                            ...
                            <>
                              <i className="fa-solid fa-credit-card" style={{ marginRight: 4 }}></i>
                              Online
                            </>
                          )}
                        </span>
                      ...
                      <td>
                        <span
                          className={`ap-badge ${STATUS_BADGE[item.statusOrderData?.value] || 'ap-badge-gray'}`}
                        >
                      ...
                      <td
                        style={{
                          textAlign: 'right',
                          fontWeight: 700,
                          color: isSuccess ? '#10b981' : isCancelled ? '#ef4444' : '#fbbf24',
                        }}
                      >
                        {isCancelled && '-'}
