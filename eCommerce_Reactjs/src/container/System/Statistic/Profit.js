      ...
      try {
        ...
        if (res?.errCode === 0) {
          ...
          const exported = res.data.map((item) => {
            ...
            return {
              id: item.id,
              createdAt: moment.utc(item.createdAt).local().format('DD/MM/YYYY HH:mm'),
          ...
          setData(res.data);
          setDataExport(exported);
          setStats({ actualRevenue, actualImport, actualProfit, deliveredOrders });
        }
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="ap-page">
      <PageHeader
        title={
          <>
            <i className="fa-solid fa-chart-pie" style={{ marginRight: 8 }}></i>Thống kê lợi nhuận
          </>
        }
        subtitle="Phân tích lợi nhuận theo ngày / tháng / năm"
        actions={
          data.length > 0 && (
            <button
              className="ap-btn ap-btn-success"
              onClick={() => CommonUtils.exportExcel(dataExport, 'Lợi nhuận', 'Profit')}
            >
      ...
      {data.length > 0 && (
        ...
        >
          ...
          <span>
            <strong>Lưu ý:</strong> Báo cáo Lợi nhuận Ròng dưới đây chỉ được tính toán dựa trên các
            đơn hàng có trạng thái <strong>"Đã giao"</strong> thành công.
          </span>
        </div>
      )}
          ...
          <table className="ap-table" style={{ minWidth: 900 }}>
            <thead>
              <tr>
                ...
                <th style={{ textAlign: 'right' }}>Tiền nhập</th>
                ...
                data.map((item, idx) => {
                  ...
                  return (
                    <tr
                      ...
                      style={{ animationDelay: `${idx * 20}ms`, opacity: isSuccess ? 1 : 0.6 }}
                    >
                      <td
                        style={{
                      ...
                      <td>
                        <span
                          className={`ap-badge ${isSuccess ? 'ap-badge-green' : isCancelled ? 'ap-badge-red' : 'ap-badge-amber'}`}
                        >
                          {status}
                        </span>
                      </td>
                      <td style={{ textAlign: 'right', color: '#fbbf24', fontWeight: 600 }}>
                        {isCancelled && '-'}
                        {fmt(item.totalpriceProduct)}
                      </td>
                      <td style={{ textAlign: 'right', color: 'var(--ap-text-muted)' }}>
                        {isCancelled && '-'}
                      ...
                      >
                        {!isSuccess ? 'Không tính' : fmt(profit)}
                      </td>
                      <td>
                        <Link
