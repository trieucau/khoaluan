...
const ManageProductDetail = () => {
  ...
  const handleDelete = async (detailId) => {
    ...
    if (res?.errCode === 0) {
      ...
      loadData(page * PAGINATION.pagerow);
  ...
  const discount = (orig, disc) => {
    ...
    return pct > 0 ? (
      <span className="ap-badge ap-badge-green" style={{ marginLeft: 6 }}>
        -{pct}%
      </span>
    ) : null;
  };
      ...
      <PageHeader
        ...
        actions={
          <div className="d-flex gap-2">
            ...
            <Link to={`/admin/add-product-detail/${id}`} className="ap-btn ap-btn-primary">
              + Thêm phân loại
            </Link>
          </div>
        }
                  ...
                  >
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
                        ...
                        >
                          <i className="fa-solid fa-image" style={{ marginRight: 8 }}></i>Ảnh
                        </Link>
                        <Link
                          to={`/admin/update-product-detail/${item.id}`}
                          className="ap-btn ap-btn-ghost ap-btn-sm"
                        >
