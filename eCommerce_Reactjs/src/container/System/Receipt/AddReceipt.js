...
const AddReceipt = () => {
  ...
  const handleSave = async () => {
    if (!values.quantity || !values.price || !sizeId) {
      toast.error('Vui lòng điền đầy đủ thông tin');
      return;
    }
    setLoading(true);
    try {
      const res = await createNewReceiptService({
        supplierId: values.supplierId,
        userId: user.id,
        productDetailSizeId: sizeId,
      ...
      <div className="ap-card" style={{ maxWidth: 760 }}>
        <div className="ap-card-header">
          <span className="ap-card-title">
            <i className="fa-solid fa-list-check" style={{ marginRight: 8 }}></i>Thông tin phiếu
            nhập
          </span>
        </div>
        <div className="ap-card-body">
          ...
          <div
            style={{ borderTop: '1px solid var(--ap-border)', margin: '16px 0', paddingTop: 16 }}
          >
            <div
              style={{
                fontSize: 13,
            ...
            >
              <i className="fa-solid fa-bag-shopping" style={{ marginRight: 8 }}></i>Chọn sản phẩm
              nhập
            </div>
