...
const ShipperDashboard = ({
  ...
  const { isOnline, gpsStartTime, gpsTotalMs } = gpsData || {
    isOnline: false,
    gpsStartTime: null,
    gpsTotalMs: 0,
  };
  ...
  const mapData = useMemo(() => {
    ...
    visibleDestinations.forEach((dest, idx) => {
      markers.push({
        ...
        icon: createNumberIcon(selectedOrderId ? '!' : idx + 1),
        label: `Đơn #${dest.id}`,
      });
    });
  ...
  if (loading)
    return (
      <div className="sp-page">
        <div className="sp-skeleton" style={{ height: '80vh' }} />
      </div>
    );
  ...
  return (
    <div className="sp-command-center">
      ...
      <div
        className="sp-overlay-container"
      ...
      >
        ...
        <div className="sp-float-middle">
          ...
          >
            ...
            >
              <WeatherWidget
                dragHandleClass={isMobile ? '' : 'sp-drag-handle'}
                pos={shipperPos}
                weatherData={weatherData}
                isMobile={isMobile}
              />
          ...
          {isMobile && (
            <div
              ...
              onClick={() => {
                if (!dragging) setIsHubExpanded(!isHubExpanded);
              }}
              style={{
                position: 'fixed',
                left: Math.min(hubPos.x, window.innerWidth - 70),
                top: Math.min(hubPos.y, window.innerHeight - 150),
                width: 60,
                height: 60,
                borderRadius: '50%',
                background: 'linear-gradient(135deg, #3b82f6, #06b6d4)',
            ...
            >
              {isHubExpanded ? <IconClose /> : <IconGear />}
            </div>
          )}
                ...
                <div style={{ padding: 24, overflowY: 'auto', flex: 1, minHeight: 0 }}>
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ fontSize: 11, color: '#94a3b8' }}>
                        {heroOrder.addressUser?.phoneNumber}
                      </div>
                    </div>
                    <div
                      style={{
                    ...
                    >
                      ...
                      <div
                        style={{ fontSize: 13, fontWeight: 700, color: '#10b981', marginBottom: 4 }}
                      >
                        {heroOrder.paymentData?.value === 'Tiền mặt'
                          ? 'Thu hộ (COD)'
                          : 'Đã thanh toán'}
                      </div>
                      <div style={{ fontSize: 11, color: '#94a3b8' }}>
                        Qua {heroOrder.paymentData?.value || 'Ví/Thẻ'}
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ marginBottom: 24, padding: '0 8px' }}>
                    ...
                    >
                      ...
                      />
                      <div
                        style={{ fontSize: 12, fontWeight: 700, color: '#fff', marginBottom: 2 }}
                      >
                        Đã lấy hàng
                      </div>
                      <div style={{ fontSize: 11, color: '#64748b', marginBottom: 16 }}>
                        {moment(heroOrder.updatedAt).format('HH:mm - DD/MM/YYYY')}
                      </div>
                      ...
                      />
                      <div
                        style={{ fontSize: 12, fontWeight: 700, color: '#fbbf24', marginBottom: 2 }}
                      >
                        Đang giao đến khách
                      </div>
                      <div style={{ fontSize: 11, color: '#64748b' }}>
                        Đang trên lộ trình tối ưu
                      </div>
                    </div>
                  </div>
                        ...
                        <>
                          ...
                          >
                            ...
                            >
                              ...
                              <span style={{ fontSize: 13, color: '#fff', fontWeight: 700 }}>
                                {formatMoney(heroOrder.typeShipData?.price || 0)}
                              </span>
                            </div>
                            <div
                              style={{
                            ...
                            >
                              ...
                              <div style={{ textAlign: 'right' }}>
                                ...
                                >
                                  {formatMoney(
                                    totalProductPrice + (heroOrder.typeShipData?.price || 0)
                                  )}
                                </div>
                                <div
                                  style={{
