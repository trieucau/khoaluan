...
const RETURN_TABS = [
  ...
  { key: 'S7', label: 'Đã hủy', icon: (<svg className="sp-icon-xs" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" /><line x1="15" y1="9" x2="9" y2="15" /><line x1="9" y1="9" x2="15" y2="15" /></svg>) },
  { key: 'S8', label: 'Giao thất bại', icon: (<svg className="sp-icon-xs" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" /></svg>) },
];
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (res?.errCode === 0) {
        ...
        const returnOrders = (res.data || []).filter(o => o.statusId === 'S7' || o.statusId === 'S8');
  ...
  return (
    <div className="sp-page" style={{ maxWidth: 800, margin: '0 auto', paddingBottom: 100 }}>
      <div className="sp-page-header">
        ...
        <div className="sp-page-subtitle">Danh sách các đơn hàng đã hủy hoặc giao thất bại cần xử lý hoàn trả</div>
      </div>
                  ...
                  return (
                    <tr key={o.id} className="sp-row-enter" style={{ animationDelay: `${i * 40}ms` }}>
                      ...
                      <td style={{ color: 'var(--sp-text-muted)', fontSize: 12 }}>
                        {moment(o.updatedAt).format('DD/MM/YYYY')}<br />
                        <span style={{ color: 'var(--sp-text-dim)' }}>{moment(o.updatedAt).format('HH:mm')}</span>
                      </td>
                      <td>
