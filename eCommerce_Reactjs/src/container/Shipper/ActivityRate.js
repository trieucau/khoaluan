...
const MetricCard = ({ title, value, unit, color, icon, description }) => (
  <div
    className="sp-glass-panel sp-metric-card"
    style={{ padding: '20px', borderRadius: '20px', flex: 1, minWidth: '140px' }}
  >
    <div
      style={{
        display: 'flex',
      ...
      >
        ...
        >
          <div className="sp-gauge-container">
            <svg className="sp-gauge-svg" width="200" height="200" viewBox="0 0 100 100">
              ...
              <circle
                ...
                strokeDashoffset={282.7 - (282.7 * stats.score) / 100}
          ...
          <div style={{ marginTop: 24, textAlign: 'center' }}>
            <div
              className={`sp-badge ${rankDetails.badge}`}
              style={{ padding: '6px 16px', borderRadius: 20, fontSize: 13, fontWeight: 700 }}
            >
              {rankDetails.label}
            </div>
            <div
        ...
        <div className="sp-weekly-chart">
          {weeklyData.map((d, i) => (
            <div key={d.day} className="sp-chart-col">
              <div
                className={`sp-chart-bar${i === 4 ? ' active' : ''}`}
                style={{ height: `${d.val}%` }}
              >
                {i === 4 && <div className="sp-chart-bar-val">{d.val}</div>}
              </div>
              <div style={{ fontSize: 11, color: 'var(--sp-text-muted)', fontWeight: 600 }}>
                {d.day}
              </div>
            </div>
          ))}
