  ...
  return (
    <div className="sp-glass-panel" style={{ 
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: isMobile ? 0 : 4 }}>
        ...
        <div>
          <div style={{ fontSize: isMobile ? 13 : 12, fontWeight: 800, color: '#fff' }}>{weather.temp}°C {isMobile ? '' : `• ${weather.city}`}</div>
          <div style={{ fontSize: isMobile ? 10 : 9, color: 'var(--sp-text-dim)', fontWeight: 600 }}>{isMobile ? weather.city + ' • ' : ''}{weather.desc}</div>
        </div>
      </div>
    </div>
  );
