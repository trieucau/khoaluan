...
const ShipperStatusBar = ({
  ...
  const tickerText = React.useMemo(() => {
    ...
    if (weatherData) {
      ...
      if (code >= 51)
        msg.push('🌧️ Cảnh báo mưa: Đường trơn trượt, hãy giảm tốc độ và chú ý an toàn.');
    ...
    if (dailyStats.count > 0) {
      msg.push(
        `✨ Hôm nay: Đã hoàn thành ${dailyStats.count} đơn • Thu nhập: +${formatMoney(dailyStats.income)}`
      );
    } else {
