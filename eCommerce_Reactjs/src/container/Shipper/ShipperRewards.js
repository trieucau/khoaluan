    ...
    return (
      <div className="sp-page">
        ...
        <div className="sp-stats-grid">
          {[1, 2, 3].map((i) => (
            <div key={i} className="sp-skeleton" style={{ height: 200, borderRadius: 16 }} />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="sp-page sp-rewards-page">
      <div className="sp-page-header">
        <div className="sp-page-title">Chương trình thưởng</div>
        <div className="sp-page-subtitle">
          Tích lũy điểm thưởng từ mỗi đơn hàng và đổi những phần quà giá trị
        </div>
      </div>
        ...
        >
          <div style={{ flex: '1 1 200px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 12 }}>
              ...
              <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--sp-primary-light)' }}>
                ĐIỂM
              </span>
            </div>
            <div style={{ marginTop: 8, fontSize: 13, color: 'var(--sp-text-muted)' }}>
              ...
              <span
                style={{
                  color: score > 80 ? 'var(--sp-success)' : 'var(--sp-warning)',
                  fontWeight: 700,
                }}
              >
                {score}%
          ...
          <div style={{ flex: '2 1 300px' }}>
            ...
            >
              <div>
                <span
                  className={`sp-badge sp-badge-${currentTier?.color || 'blue'}`}
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: 11, color: 'var(--sp-text-dim)', fontWeight: 700 }}>
                  HẠNG KẾ TIẾP
                </div>
      ...
      <div style={{ marginBottom: 20 }}>
        <div className="sp-tabs" style={{ maxWidth: 500 }}>
          <button
            className={`sp-tab${tab === 'redeem' ? ' active' : ''}`}
          ...
          <button
            className={`sp-tab${tab === 'perks' ? ' active' : ''}`}
          ...
          <button
            className={`sp-tab${tab === 'history' ? ' active' : ''}`}
        ...
        {tab === 'redeem' && (
          <div className="sp-quick-grid">
            {REWARDS.map((r) => (
              <div
                key={r.id}
                className={`sp-card sp-reward-card ${r.color}`}
                style={{ padding: 20, cursor: 'pointer', transition: 'all 0.3s ease' }}
              >
                <div style={{ fontSize: 32, marginBottom: 16 }}>{r.icon}</div>
                <div style={{ fontSize: 15, fontWeight: 700, color: '#fff', marginBottom: 6 }}>
                  {r.title}
                </div>
                <div style={{ fontSize: 12, color: 'var(--sp-text-muted)', marginBottom: 16 }}>
                  Loại: {r.type}
                </div>
                <div
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
                >
                  <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--sp-primary-light)' }}>
                    {r.cost} <span style={{ fontSize: 11 }}>ĐIỂM</span>
                  </div>
                  <button className="sp-btn sp-btn-sm sp-btn-primary" style={{ borderRadius: 8 }}>
                    Đổi ngay
                  </button>
                </div>
              </div>
                  ...
                  >
                    ...
                    <span
                      style={{
                        fontSize: 14,
                        fontWeight: 600,
                        color: isActive ? '#fff' : 'var(--sp-text-dim)',
                      }}
                    >
                      {p.label}
                    </span>
                ...
                <tbody>
                  {history.length === 0 ? (
                    <tr>
                      <td
                        colSpan={3}
                        style={{ textAlign: 'center', padding: 40, color: 'var(--sp-text-dim)' }}
                      >
                    ...
                    history.map((h, i) => (
                      ...
                      >
                        <td>
                          <div style={{ fontWeight: 600, color: '#fff' }}>{h.title}</div>
                        </td>
                        <td>
                          <span
                            style={{ fontSize: 15, fontWeight: 800, color: 'var(--sp-success)' }}
                          >
                            +{h.points}
                          </span>
                        </td>
