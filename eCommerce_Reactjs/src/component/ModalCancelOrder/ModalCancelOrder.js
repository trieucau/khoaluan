...
function ModalCancelOrder({ show, onConfirm, onClose }) {
  ...
  const handleConfirm = () => {
    if (!reason.trim()) { toast.warning('Vui lòng nhập lý do hủy đơn'); return; }
    ...
    <div className="sp-modal-backdrop" onClick={handleClose}>
      <div className="sp-modal" onClick={(e) => e.stopPropagation()}>
        <div className="sp-modal-header">
          ...
          <button className="sp-modal-close" onClick={handleClose}>×</button>
        </div>
        <div className="sp-modal-body">
          ...
          <textarea
            ...
            autoFocus
          />
          <div className="sp-char-count" style={{ color: reason.length > MAX * 0.9 ? 'var(--sp-warning)' : undefined }}>
            {reason.length}/{MAX}
          </div>
        </div>
        <div className="sp-modal-footer">
          <button className="sp-btn sp-btn-ghost" onClick={handleClose}>Đóng</button>
          <button
